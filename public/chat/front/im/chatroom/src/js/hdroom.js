...
var page = {
	init:function () {
		...
		$.ajax({
            cache: true,
            type: "POST",
            url: '/front/getPullUrl?chatRoomId=' + roomid+"&nocreate=1",//登录之后把userName放入cookie中
            data: {},
            xhrFields: {
            ...
            success: function (data) {
                if (data.code == 200) {
					...
					util.setCookie("nickName",data.data.exhibitorName);
					util.setCookie("nick",data.data.exhibitorName);
					//$("#zhc").html("主会场："+data.data.exhibitorName);
					$("#exhibitionName").html(data.data.exhibitorName);
				} else {
	...
	initNode:function(){
		...
		this.$closeScreeBtn = $("#closeScreeBtn");
	...
	initEvt:function(){
		...
		this.$closeScreeBtn.on("click",this.updateChatroom.bind(this));
	...
	switchTab:function(evt){
		if($(evt.target).attr("data-value")!==$("#room .j-tab.crt").attr("data-value")){
			...
			}else if($(evt.target).attr("data-value")==="linkList"){
				this.$pannel.addClass("hide");
				$(".linkList").removeClass("hide")
				$("#room .j-tab.crt").toggleClass("crt");
				$(".linkListTab").toggleClass("crt");
			}else {
				this.$pannel.addClass("hide");
				$(".chat").removeClass("hide")

				$("#room .j-tab.crt").toggleClass("crt");
				$(".chatTab").toggleClass("crt");
	...
	showMenu:function(evt){
		...
		var show = function(item){
			if(item.account){
				...
				that.$menu.find(".j-admin").text(item.type==="manager"?"解除嘉宾":"设置嘉宾");
			}
			...
			if(ownerOperate||managerOperate){
				...
				that.$menu.css({"top":y-300,"left":x-638,"display":"block"});
			}		
		}	
	...
	updateChatroom:function(){
		...
		$("#myModal").modal('hide');
			...
			this.link.updateChatroom(text,function(err,data){
				if(err){
					alert(err.message);
				}else{
					$("#roomAnnouncement").text(text);
				}
			}.bind(this))
