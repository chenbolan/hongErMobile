...
/*  $.ajax({
  ...
  function onCustomSysMsg(msg) {
    ...
    var fromAccount = content.value;
    var id = content.id;
    console.log('收到自定义通知eventType', eventType);
    console.log('收到自定义通知fromAccount', fromAccount);
    var sendMessage = "您好,请问有什么可以帮助您吗？"
    if(eventType =="sendSysMsg"){
      $.ajax({
        cache : true,
        type : "POST",
        dataType : "JSON",
        url : baseUrl + '/customservice/getDetail',
        data : {},
        xhrFields: {
          withCredentials: true
        },
