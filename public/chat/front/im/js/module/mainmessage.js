...
YX.fn.getLocalMsgsDone = function(err, data) {
  if (!err) {
    var reset = true
    var arrayMsg =  msg(data.msgs);
    this.cache.addMsgsByReverse(arrayMsg, true);
...
function msg(data){
  ...
  if(arrayMsg.length > 0){
    for(var index in arrayMsg){
      if(!arrayMsg[index].cc){
        var type = arrayMsg[index].type;
        var text = arrayMsg[index].text;
