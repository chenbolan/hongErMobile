...
var SDKBridge = function(ctr, data) {
  ...
  function onFriends(friends) {
    ...
    setTimeout(function(){
      console.log(kefuUid);
      //kefuUid = "c1201592146440066";
      var kefuArry = kefuUid.split(",");//
      var online=new Array();
      var z=0;
      for (var value in kefuArry) {
          console.log(kefuArry[value]);
         var dat = $('.panel_item[data-account="'+kefuArry[value]+'"][data-status="200"]');
        if(dat.length==1){
          online[z] = kefuArry[value];
          z++;
        }
      }
      if(online.length==0){//如果没有在线的，取一个离线的
        online[0] = kefuArry[0];
      }
      console.log("在线客服数："+online.length)
      var onindex = sum(0,online.length);
      //发送系统消息
      //applyLink(online[onindex],readCookie('uid'));
      sendMssage(online[onindex],readCookie('uid'));
      $('.panel_item[data-account="'+online[onindex]+'"]').click();
    },1000)
  }
  function sum (m,n){
...
//后台发送消息
  function sendMssage(from,to){
    $.ajax({
      cache : true,
      type : "POST",
      dataType : "JSON",
      url : frontBaseUrl + '/front/sendMessage?from='+from+"&to="+to,
      data : {},
      xhrFields: {
  ...
  //发送自定义消息
  function applyLink(toAccid,fromAccount) {
    console.info("fromAccount:" + fromAccount);
    console.info("toAccid:" + toAccid);
      var content = {
        type: 'sendSysMsg',
        value: fromAccount
      };
  ...
  function onMsg(msg) {
    ...
    console.log('收到------------tip');
    console.log('收到------------tip->'+msg);
    var type = msg.type;
    var text = msg.text;
    var that = this;
    var readCookie = util.readCookie("lang");
    ...
    if(type == "text"){
      $.ajax({
        cache: true,
        type: "POST",
        url: frontBaseUrl+'/front/trans',
        data: $('#transForm').serialize(),
        async: false,
        error: function (request) {
          console.log("接口失败");
          that.controller.doMsg(msg);
        },
        success: function (data) {
          if (data.code==200) {
            if(text == data.data){
