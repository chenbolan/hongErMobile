...
/***/ function(module, exports, __webpack_require__) {
	...
	var SessionList = function(options){
		...
		this._body.className = options.clazz||"m-panel" +" j-session"  + " friends_con_mobile";
...
/***/ function(module, exports, __webpack_require__) {
	...
	var FriendList = function(options){
		...
		this.provider = options.infoprovider;
		this._body = document.createElement('ul');
		this._body.className = options.clazz||"m-panel" +" j-friend" + " friends_con_mobile";
	      ...
	      if (window.CONFIG && window.CONFIG.openSubscription) {
	        ...
	        if (personSubscribes[account] && personSubscribes[account][1]) {
	          ...
	          multiPortStatus = '[' + multiPortStatus + ']'
	            ...
	            html += ['<li class="panel_item '+(info.crtSession===info.target?'active':'')+'" data-scene="p2p" data-status="'+status+'" data-account="' + info.account + '">',
	                        '<div class="panel_avatar"><img class="panel_image" src="'+info.avatar+'"/></div>',
	                        '<div class="panel_text">',
	                            // '<p class="panel_single-row">'+info.nick + ' ' + multiPortStatus +'</p>',
	                            `<p class="panel_single-row"><span class="friend_name">${info.nick}</span><span class="login_status">${multiPortStatus}</span></p>`,
	                        '</div>',
	                    '</li>'].join("");
			}
		}
