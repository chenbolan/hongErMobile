	...
	var Mapplic = function(element) {
		...
		function Lightbox() {
			...
			this.init = function() {
				...
				this.link = $('<a>' + self.loc.more + '</a>').addClass('mapplic-popup-link').attr('href', '#').attr('target','_blank').hide().appendTo(this.el);
				if (self.o.linknewtab) this.link.attr('target', '_blank');
		...
		function Sidebar() {
			...
			this.search = function() {
				//把隐藏的location先设置为可见
				$(".mapplic-list-location").css('display','block');
				...
				if(keyword.length <= 0 && $(".mapplic-tag").length <= 0){
